...
export default function ChallengePage() {
  ...
  const startChallenge = async () => {
    ...
    try {
      ...
      else toast.error(msg || 'Failed to load challenge');
          ...
          <motion.div key="select" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <div className="card p-4 sm:p-6 space-y-5">
              ...
              {noQuestions && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">Try a different category or ask an admin to add questions.</p>
                  </div>
                </motion.div>
              )}
              <div className="p-4 rounded-xl bg-brand-50 dark:bg-arena-700 border border-brand-100 dark:border-arena-600">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">All correct → 100 XP + 50 points + 10% discount coupon</p>
              </div>
          ...
          <motion.div key={`q-${qIndex}`} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <div className="card p-4 sm:p-6 space-y-5">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Question {qIndex + 1} / {challenge!.questions.length}</span>
              ...
              <div className="space-y-3">
                {currentQ.options.map((opt, i) => (
                  <motion.button key={i} onClick={() => handleSelect(i)} disabled={selected !== null} whileTap={{ scale: 0.98 }}
                    ...
                    <span className="font-bold mr-3 text-brand-400">{String.fromCharCode(65 + i)}.</span>{opt}
                  </motion.button>
                ))}
              </div>
            </div>
            ...
            <div className="card p-6 sm:p-8 text-center space-y-5">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                  {result.allCorrect ? 'Perfect Score!' : `${result.score} / ${result.total}`}
                </h2>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                  {result.allCorrect ? 'You nailed every question!' : 'Keep practising — you\'ll get there!'}
                </p>
              </div>
              {(result.xpAwarded > 0 || result.pointsAwarded > 0) && (
                <div className="flex justify-center gap-6">
                  {result.xpAwarded   > 0 && <div className="text-center"><p className="text-2xl font-black text-brand-500">+{result.xpAwarded}</p><p className="text-xs text-gray-500">XP</p></div>}
                  {result.pointsAwarded > 0 && <div className="text-center"><p className="text-2xl font-black text-yellow-500">+{result.pointsAwarded}</p><p className="text-xs text-gray-500">Points</p></div>}
                </div>
              )}
              {result.discountCoupon && (
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl p-4">
                  <p className="text-sm font-semibold text-green-700 dark:text-green-400">🎁 Discount Coupon Earned!</p>
