...
interface LeaderboardEntry {
  ...
  level: number; xp: number; score: number; rank: number;
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Compete with players across the city</p>
      </div>
      ...
      <div className="flex gap-2 bg-gray-100 dark:bg-arena-800 p-1 rounded-xl">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={cn('flex-1 py-2 px-2 sm:px-3 rounded-lg text-xs sm:text-sm font-medium transition-all',
              tab === t.key ? 'bg-white dark:bg-arena-700 text-gray-900 dark:text-gray-100 shadow-sm'
          ...
          {[entries[1], entries[0], entries[2]].map((entry, podiumIdx) => {
            ...
            return (
              <motion.div key={entry._id}
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: podiumIdx * 0.1 }}
                className={cn('flex flex-col items-center justify-end gap-2 p-3 rounded-2xl border bg-gradient-to-b', RANK_COLORS[rank - 1], heights[podiumIdx])}>
                ...
                <div className="text-center">
                  <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate max-w-[80px]">{entry.name?.split(' ')[0]}</p>
                  <p className="text-xs text-gray-500">{formatNumber(entry.score)}</p>
                </div>
                {RANK_ICONS[rank - 1]}
              </motion.div>
            );
        ...
        {loading ? (
          Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="flex items-center gap-4 p-4 animate-pulse">
              ...
              <div className="h-4 bg-gray-200 dark:bg-arena-700 rounded w-16 ml-auto" />
            </div>
          ))
        ) : entries.length === 0 ? (
          ...
          entries.map((entry, i) => {
            ...
            return (
              <motion.div key={entry._id}
                ...
                className={cn('flex items-center gap-4 px-4 py-3', isMe && 'bg-brand-50 dark:bg-brand-900/10')}>
                <div className="w-7 text-center">
                  {i < 3 ? RANK_ICONS[i] : <span className="text-sm font-bold text-gray-400">#{i + 1}</span>}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn('text-sm font-semibold truncate', isMe ? 'text-brand-600 dark:text-brand-400' : 'text-gray-900 dark:text-gray-100')}>
                    {entry.name} {isMe && <span className="text-xs font-normal">(you)</span>}
                  </p>
                  <p className="text-xs text-gray-400">Level {entry.level}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-gray-900 dark:text-gray-100">{formatNumber(entry.score)}</p>
                  <p className="text-xs text-gray-400">{scoreSuffix(tab)}</p>
                </div>
              </motion.div>
            );
          })
