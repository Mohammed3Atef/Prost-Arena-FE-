      ...
      <section className="relative overflow-hidden rounded-3xl bg-arena-gradient py-16 px-8 text-white text-center">
        ...
        <div className="relative z-10">
          <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="inline-block badge-brand mb-4 text-sm">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            ...
            Order food, crush challenges, spin the wheel, and climb the leaderboard.
            Every meal is an adventure.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            className="flex flex-wrap items-center justify-center gap-4">
            <Link href="/menu" className="btn-primary text-lg px-8 py-4 animate-pulse-glow">
          ...
          {user && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
              className="mt-8 max-w-sm mx-auto card-glass p-4 text-left">
              <div className="flex items-center gap-3 mb-3">
                <div className="level-ring w-12 h-12 text-lg font-bold font-display">{user.level}</div>
                <div>
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {FEATURES.map((f) => (
            <motion.div key={f.href} variants={item}>
              <Link href={f.href} className="card flex flex-col items-center text-center p-6 gap-3 group">
      ...
      <section className="bg-arena-gradient rounded-3xl p-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-white text-center">
          {[
            ...
            { icon: '🍽️', label: 'Menu Items', value: '200+' },
          ].map((s, i) => (
